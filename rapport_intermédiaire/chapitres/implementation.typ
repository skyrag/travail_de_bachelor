...
#v(2em)
 ...
 En ce qui concerne la configuration de Play certain de nos service demande une certaine configuration notamment notre Db et Argon2. Tout est présent dans notre fichier de configuration de l'application et il y a aussi un .env pour des données potentiellement plus sensible comme les secrets.
#v(1em)
...
La création d'une partie n'a pas été implémenté dans le gameRepository car on a estimé que c'est un comportement qui n'a pas lieux d'être et ne doit être utilisé qu'a un seul endroit, d'où son isolement.
...
À chaque étape, l'algorithme retire de la liste des cases à explorer (open list) celle ayant le score f le plus bas, et explore ses voisines. Une fois qu'une case a été explorée, elle est marquée comme définitivement traitée (closed list) et ne sera plus reconsidérée, sauf si un chemin strictement plus court vers elle est découvert par la suite. L'algorithme s'arrête dès que la case d'arrivée est retirée de la open list, garantissant qu'aucun chemin plus court n'existe. Le chemin final est ensuite reconstruit en remontant, de case en case, le "parent" de chaque case jusqu'au point de départ.
#v(1em)
...
Le matchmaking doit maintenir une liste des joueurs en attente ainsi que leur connexion associée, afin de pouvoir démarrer une partie une fois qu'un nombre suffisant de joueurs est atteint.
#v(1em)

Les contrôleurs de Play sont par nature concurrents, et c'est déjà un problème que nous avons mentionné dans la section concernant l'*ActeurMonitor*.
#v(1em)
...
#v(2em)
Comme expliqué précédemment, notre jeu repose sur de nombreux mécanismes aléatoires, mais on souhaite pouvoir proposer un système de replay des parties. Pour que ce dernier soit fiable, il est nécessaire de stocker les graines (seeds) utilisées, afin que chaque décision aléatoire, une fois rejouée, mène exactement aux mêmes conséquences.
...
On n'a pas voulu réutiliser notre service de hashing basé sur Argon2 pour cet usage : cet algorithme est volontairement conçu pour être lent et gourmand en ressources (afin de ralentir les attaques par force brute sur les mots de passe), ce qui le rend inadapté à une génération de seed devant rester rapide, et il nécessite en outre un sel non réutilisable tel quel dans un contexte déterministe. Nous nous sommes donc tournés vers SHA-256, une fonction de hash cryptographique rapide et surtout parfaitement déterministe : pour une même entrée, elle produit toujours exactement la même sortie, ce qui est précisément la propriété recherchée pour garantir la reproductibilité de nos seeds lors d'un replay.

=== Test

La validation du système repose sur une suite de tests unitaires écrite avec JUnit 5, complétée par Mockito pour l'isolation des dépendances et par le module ActorTestKit de Pekko pour les composants concurrents. Les structures de données du module de pathfinding (HexPosition, Tile, HexGrid) et l'algorithme $A^*$ qui les exploite ont été testés de manière déterministe : les tuiles sont reliées manuellement en dehors de toute grille réelle, ce qui permet de contrôler précisément l'ordre de traitement de la file de priorité et de couvrir des cas limites difficiles à provoquer autrement, tels que la mise à jour d'un nœud encore ouvert lorsqu'un chemin moins coûteux est découvert, ou l'ignorance d'un nœud déjà fermé. Cette démarche a par ailleurs permis d'identifier une erreur de bornes dans la construction de la grille hexagonale, corrigée avant l'écriture des tests correspondants. Les composants asynchrones, le gestionnaire de connexions (ActeurMonitor) et les acteurs typés (ConnexionActor, GameActor), ont fait l'objet d'une stratégie différente : les dépendances externes (persistance, services métier, système d'acteurs) sont simulées via Mockito, tandis qu'un système d'acteurs de test réel est instancié pour observer le comportement effectif des échanges de messages, notamment la gestion du heartbeat, le tampon de reconnexion et la propagation des actions de jeu entre les acteurs. Cette approche privilégie la vérification du comportement observable plutôt que de l'état interne, difficilement accessible sur des acteurs encapsulant fortement leurs données.

La couche service métier (GameLevelService, HashService, MatchmakingService, SeedMakerService, SimulationService) a fait l'objet d'une isolation systématique de ses dépendances externes : JPAApi et Config sont simulés via Mockito pour contrôler le chargement des données de progression, tandis que le service de hachage est validé par exécution réelle de l'algorithme Argon2id. Avec des paramètres réduits pour la performance des tests, afin de garantir que le hachage, la vérification et l'effacement des mots de passe en mémoire se comportent conformément aux spécifications, y compris en cas d'échec. Le service de génération de graines a fait l'objet d'une attention particulière : outre la vérification du déterminisme des graines dérivées, un mock statique de MessageDigest a permis de couvrir la branche défensive normalement inatteignable correspondant à l'absence de l'algorithme SHA-256 sur la JVM. Le service de matchmaking a été validé par simulation de l'arrivée successive de joueurs, vérifiant le déclenchement de la création de partie au seuil attendu ainsi que la réinitialisation correcte de la file d'attente. Le service de simulation asynchrone, enfin, a été isolé de son moteur de combat interne par interception de sa construction (mockConstruction), ce qui a permis de vérifier la délégation vers le dispatcher dédié sans dépendre du calcul réel du combat.

Le moteur de simulation de combat lui-même (FightingService, FightingContext, ainsi que les systèmes StatusSystem, EndSystem et ActionSystem) présente la particularité d'instancier directement ses collaborateurs internes, sans injection de dépendances. Pour préserver l'isolation malgré ce couplage fort, chaque construction interne (contexte de combat, systèmes de mise à jour, unités de combat, objets de résultat) a été interceptée via le même mécanisme mockConstruction, permettant de vérifier la logique d'orchestration (bornage de la boucle de simulation, conditions d'arrêt, calcul des points de vie perdus) indépendamment du comportement réel des systèmes sous-jacents. Combinée à la capture des arguments de construction, cette approche a également permis de valider EndSystem sans dépendre des accesseurs internes de ses objets d'événement, révélant au passage un déréférencement non protégé (getFirst() sur une liste d'unités vivantes potentiellement vide) ainsi qu'un champ non initialisé exposé par getWinner(), susceptible de provoquer une exception d'unboxing. Certaines branches, en particulier celles dépendant de types non exposés dans le périmètre analysé (stratégies de ciblage des capacités actives, entités de persistance liées aux comptes utilisateurs) n'ont pu être couvertes avec la même rigueur et ont été identifiées comme axes d'amélioration de la suite de tests.

Le LoginRepository, en charge de la persistance des comptes utilisateurs, a été validé par des tests d'intégration reposant sur Testcontainers plutôt que sur une isolation par mocks : une véritable instance PostgreSQL est instanciée dans un conteneur Docker éphémère, initialisée par un script SQL dédié, puis injectée dans une application Play complète via GuiceApplicationBuilder. Cette approche permet d'exercer le repository contre un moteur de base de données réel et non contre une simulation de son comportement, garantissant ainsi que le mapping objet-relationnel, les contraintes du schéma et les requêtes effectivement générées se comportent comme attendu en production. Les scénarios couvrent le cycle de vie complet d'un utilisateur : création puis récupération par email et par nom d'utilisateur (avec vérification de la cohérence entre les deux résultats), création suivie d'une relecture directe de l'entité persistée, ainsi que la récupération exhaustive de l'ensemble des comptes enregistrés, y compris la vérification négative qu'un utilisateur non ajouté n'y figure pas. Chaque test prend soin de supprimer les données qu'il a introduites à son issue, afin de préserver l'indépendance des scénarios malgré le partage du même conteneur pour toute la suite. Cette stratégie, plus coûteuse en temps d'exécution qu'une suite unitaire classique, a été réservée à la seule couche de persistance, là où la fidélité au comportement réel du système de gestion de base de données prime sur la rapidité d'exécution.

Avec tous ces tests on obtiens seulement $46%$ de code coverage et $34%$ de branche coverage. Il sera donc nécessaire dans le future de tester beaucoup plus surtout les acteur et les entités qui sont les deux grosse partie manquante de notre coverage.