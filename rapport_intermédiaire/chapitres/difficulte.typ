...
#v(2em)
On va ici parler des difficultés rencontrées lors de ce travail de bachelor. C'est une partie importante, car il s'agit de défis qu'on avait pas anticipés, et dont il est essentiel de tirer un maximum d'enseignements afin de ne plus y être confrontés à l'avenir, mais surtout afin de progresser.
#v(2em)
== Première base de données
#v(2em)
Abordons tout d'abord la conception et l'implémentation de notre base de données. Lors de la conception, on avait décidé de nous orienter vers une base de données relationnelle. Cependant, parmis les cours que j'ai suivi durant mon cursus ceux parlant de base de donnée avait quelques lacunes quant a l'aspect pratique de la conceptualisation et l'implémentation d'un base de donnée. Je me suis donc retrouvé à devoir rattraper le cours de Base de Données Relationnelles de la HEIG afin de combler mes lacunes sur le sujet.
#v(1em)

Ce rattrapage a entraîné un ralentissement, aussi bien au niveau de la conception que de l'implémentation. C'est notamment à cette occasion que j'ai découvert le fonctionnement de l'héritage dans une base de données relationnelle, ainsi que la manière de garantir la cohérence de cet héritage à l'aide de triggers.
...
#v(2em)
Lors de la phase initiale et de l'état de l'art, n'ayant encore jamais eu à faire de choix d'architecture ou de technologies dans les différents projets auxquels j'avais participé jusqu'ici (principalement des laboratoires), j'ai perdu beaucoup de temps à m'éparpiller sur des aspects peu utiles, qui étaient soit trop poussés pour le périmètre de mon projet, soit inutiles au vu de sa nature ou du framework choisi.
#v(1em)
